...
// Stamps league_id onto a row being inserted.
//
// Omits the key entirely when leagueId is null -- that is the pre-migration
// case, where the column does not exist yet and sending it would make every
// insert fail. Once the migration has run, leagueId is always a real id.
export function withLeague(payload, leagueId) {
  return leagueId ? { ...payload, league_id: leagueId } : payload
}
