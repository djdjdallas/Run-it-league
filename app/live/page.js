...
import { sampleStreams, sampleGames, sampleTeams } from "@/lib/sample-data"
import { formatDate, formatTime } from "@/lib/utils"

export default function LivePage() {
  const [selectedStream, setSelectedStream] = useState(null)

  // Get streams with game info
  const streams = sampleStreams.map((stream) => {
    const game = sampleGames.find((g) => g.id === stream.game_id)
    return { ...stream, game }
  })
